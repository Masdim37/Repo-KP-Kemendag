...
<head>
    ...
    <title>Tambah Referensi Unit Eselon II | Penelitian RKA-K/L</title>
...
<body>
    ...
    <div class="app-shell">
        @include('partials.sidebar', [
            'activeMenu' => 'reference-unit-eselon-2',
                    ...
                    <section class="main-card">
                        <div class="card-header">
                            <div class="card-header-left">
                                ...
                                <div>
                                    <h1 class="card-title">Tambah Referensi Unit Eselon II</h1>
                                    <p class="card-description">Tambahkan Unit Eselon II baru pada Unit Eselon I yang
                                        sesuai.</p>
                                </div>
                            </div>
                        ...
                        <form id="referenceForm" class="main-form"
                            action="{{ route('referensi.organisasi.unit2.store') }}" method="POST" novalidate>
                            @csrf
                            <div class="section-title">
                                <div>
                                    <h2>Referensi Induk</h2><span>Pilih Unit Eselon I yang menjadi induk Unit Eselon II
                                        baru.</span>
                                </div>
                            </div>
                            <div class="form-grid">
                                <div class="form-group full">
                                    <label for="unit1" class="form-label">Unit Eselon I <span
                                            class="required">*</span></label>
                                    <select id="unit1" name="kode_unit_eselon1" class="form-control">
                            ...
                            <div class="section-title">
                                <div>
                                    <h2>Data Referensi Baru</h2><span>Masukkan kode dan nama Unit Eselon II baru.</span>
                                </div>
                            </div>
                            <div class="form-grid">
                                <div class="form-group"><label for="kodeUnit2" class="form-label">Kode Unit Eselon II
                                        <span class="required">*</span></label><input type="text" id="kodeUnit2"
                                        name="kode_unit_eselon2" class="form-control" maxlength="50" autocomplete="off"
                                        value="{{ old('kode_unit_eselon2') }}" placeholder="Contoh: 09.22">
                                    <div class="field-help">Kode disimpan sebagai teks, termasuk tanda titik jika ada.
                                    </div>
                                    <div class="field-error" id="kodeUnit2Error"></div>
                                    @error('kode_unit_eselon2')
                                        <div class="field-error show">{{ $message }}</div>
                                    @enderror
                                </div>
                                <div class="form-group"><label for="namaUnit2" class="form-label">Nama Unit Eselon II
                            ...
                            <div class="status-list"><span class="status-chip" id="parentStatus">Unit Eselon
                                    I</span><span class="status-chip" id="kodeStatus">Kode Unit Eselon II</span><span
                                    class="status-chip" id="namaStatus">Nama Unit Eselon II</span></div>
                            <div class="form-actions">
                                <div class="action-message" id="actionMessage">Pilih induk dan lengkapi data referensi.
    ...
    <script>
        ...
        const form = document.getElementById('referenceForm'),
            ...
            kode = document.getElementById('kodeUnit2'),
            nama = document.getElementById('namaUnit2');
        const unit1Error = document.getElementById('unit1Error'),
            kodeError = document.getElementById('kodeUnit2Error'),
            namaError = document.getElementById('namaUnit2Error');
        ...
        function updateState(showErrors = false) {
            const p = unit1.value !== '',
                k = kode.value.trim() !== '',
                n = nama.value.trim() !== '';
            parentStatus.classList.toggle('complete', p);
            kodeStatus.classList.toggle('complete', k);
            namaStatus.classList.toggle('complete', n);
            if (showErrors) {
                setFieldError(unit1, unit1Error, p ? '' : 'Unit Eselon I wajib dipilih.');
                setFieldError(kode, kodeError, k ? '' : 'Kode Unit Eselon II wajib diisi.');
                setFieldError(nama, namaError, n ? '' : 'Nama Unit Eselon II wajib diisi.')
            } else {
                if (p) setFieldError(unit1, unit1Error, '');
                if (k) setFieldError(kode, kodeError, '');
                if (n) setFieldError(nama, namaError, '')
            }
            const valid = p && k && n;
            saveButton.disabled = !valid;
            actionMessage.textContent = valid ? 'Data siap disimpan.' : 'Pilih induk dan lengkapi data referensi.';
            return valid
        }
        [unit1, kode, nama].forEach(el => el.addEventListener(el.tagName === 'SELECT' ? 'change' : 'input', () =>
            updateState(false)));
        form.addEventListener('submit', async event => {
            ...
            DocumentProcessingModal.showLoading({
                ...
                message: 'Data Unit Eselon II sedang divalidasi dan disimpan ke database.'
            });
            try {
                ...
                if (!response.ok || payload.success === false) {
                    if (payload.errors?.kode_unit_eselon1) setFieldError(unit1, unit1Error, payload.errors
                        .kode_unit_eselon1[0]);
                    if (payload.errors?.kode_unit_eselon2) setFieldError(kode, kodeError, payload.errors
                        .kode_unit_eselon2[0]);
                    if (payload.errors?.nama_unit_eselon2) setFieldError(nama, namaError, payload.errors
                        .nama_unit_eselon2[0]);
                ...
                DocumentProcessingModal.showSuccess({
                    ...
                    message: payload.message || 'Unit Eselon II berhasil ditambahkan.',
                    buttonText: 'OKE',
                    onClose: () => {
                        kode.value = '';
                        nama.value = '';
